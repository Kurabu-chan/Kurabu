...
// docker buildx build --load -t kurabu/auth-api -f ../docker/Dockerfile ../../../
// docker buildx build --push -t rafaeltab/kurabu.auth-api:latest -t rafaeltab/kurabu.auth-api:<version> -f ../docker/Dockerfile ../../../
// docker buildx build --push -t rafaeltab/kurabu.auth-api:prod -t rafaeltab/kurabu.auth-api:<version> -t rafaeltab/kurabu.auth-api:latest -f ../docker/Dockerfile ../../../
(async () => {
    ...
    if (context === "development") {
        dockerArgs.unshift("-t", "kurabu/auth-api");
        dockerArgs.unshift("--load");
        
        process.exit(await startBuild(dockerArgs));
    } else {
        // tag with latest and package json version
        dockerArgs.unshift("-t", "rafaeltab/kurabu.auth-api:latest", "-t", `rafaeltab/kurabu.auth-api:${version}`);
        // build for all available platforms
        dockerArgs.unshift("--platform=linux/amd64,linux/arm64/v8")

        if (context === "production") {
            dockerArgs.unshift("-t", "rafaeltab/kurabu.auth-api:production");
        }
