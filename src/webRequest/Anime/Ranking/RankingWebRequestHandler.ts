...
import {
	RankingWebRequestResult,
	RankingWebRequestResultType,
} from "./RankingWebRequestResult";
...
@autoInjectable()
export class RankingWebRequestHandler
	implements IWebRequestHandler<RankingWebRequest, RankingWebRequestResult> {
	async handle(query: RankingWebRequest): Promise<RankingWebRequestResult> {
		...
		let json:
			| ListPagination<RankingWebRequestResultType>
			| ErrorResponse = data;
